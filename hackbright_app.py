...
def grades_by_title(project_title):
    query = """SELECT project_title, student_github, first_name, last_name, grade FROM Grades INNER JOIN Students on student_github = github WHERE project_title = ?""" 
    DB.execute(query, (project_title,))
    rows = DB.fetchall()
    return rows
...
def main():
    ...
    command = None
    while command != "quit":
        input_string = raw_input("HBA Database> Use commas to seperate inputs!")
        ...
        if command == "student":
            ...
            show_grades(*args)
        elif command == "all_project":
            grades_by_title(*args)
